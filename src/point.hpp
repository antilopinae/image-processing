...
struct Point : PointBase<double> {
    ...
    [[nodiscard]] auto Dot(const Point &o) const noexcept -> double
    {
        return x * o.x + y * o.y;
    }
    [[nodiscard]] auto Cross(const Point &o) const noexcept -> double
    {
        return x * o.y - y * o.x;
    }
    [[nodiscard]] auto Len() const noexcept -> double
    {
        return hypot(x, y);
    }
    ...
    [[nodiscard]] auto ToPoint2D() const -> Point2D
    {
        ...
        return Point2D{static_cast<size_t>(std::floor(x)), static_cast<size_t>(std::floor(y))};
    }
};
...
struct Point3 : Point {
    ...
    [[nodiscard]] auto Dot(const Point3 &o) const noexcept -> double
    {
