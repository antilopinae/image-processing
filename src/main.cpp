...
auto DoSmthWithImage(const std::string &im_name,
                     const std::function<void(improcessing::Image &)> &payload,
                     int width  = 600,
...
int main(int argc, char *argv[])
{
    ...
    command_lab5.add_argument("-k", "--perspective-k")
        .scan<'g', double>()
        .store_into(k_param)
        .default_value(200.0)
    ...
    command_lab5.add_argument("-n", "--frames")
        .scan<'i', int>()
        .store_into(frames)
        .default_value(36)
        .help("Number of frames for animation");

    command_lab5.add_argument("-o", "--output-prefix")
        .required()
        .store_into(output)
        .help("Output filename prefix (e.g. 'frame')");
    ...
    command_hw2.add_argument("--center")
        .nargs(2)
        .scan<'g', double>()
        .default_value(std::vector<double>{300, 300})
        .help("Center X Y");

    command_hw2.add_argument("-r", "--radius").scan<'g', double>().store_into(arc_r).default_value(200.0);

    command_hw2.add_argument("-s", "--start")
        .scan<'g', double>()
        .store_into(arc_start)
        .default_value(0.0)
        .help("Start angle (deg)");

    command_hw2.add_argument("-e", "--end")
        .scan<'g', double>()
        .store_into(arc_end)
        .default_value(270.0)
        .help("End angle (deg)");
    ...
    auto initialize_cmd = [&] -> std::optional<Command> {
        if (parser.is_subcommand_used(command_lab1)) {
            if (command_lab1.is_subcommand_used(command_lab1_circle_gray)) {
                return Laboratory1GrayCircle{.width           = 512,
                                             .height          = 512,
                                             .radius_fraction = 0.45,
                                             .output_filename = output};
            } else if (command_lab1.is_subcommand_used(command_lab1_blending)) {
                return Laboratory1Blend{.first_filename  = input_first,
            ...
            return Laboratory5{.output_prefix = output,
                               .k             = k_param,
                               .axis_x        = axis[0],
                               .axis_y        = axis[1],
                               .axis_z        = axis[2],
            ...
            return Homework2{.output_filename = output,
                             .center_x        = c[0],
                             .center_y        = c[1],
                             .radius          = arc_r,
    ...
    const auto v = overloaded{
        ...
        [](const Laboratory3 &c) -> std::expected<void, boost::system::error_code> {
            ...
            fmt::print("Saved an image with drawn polygon edges: {}\n",
            ...
            fmt::print("Saved an image with filled polygons: {}\n",
            ...
            fmt::print("Determine polygon as {} and {}\n",
                       simple ? "simple" : "complex (self-intersecting)",
            ...
            res = DoSmthWithImage(
                "lab3_degenerate_fill.png",
                [](Image &img) {
                    img.ResizeRgb(400, 200);
                    std::vector<Point2D> degeneratePoly = {
                        { 50, 100},
                        {350, 100},
                        {200, 100}
                    };
        ...
        [](const Laboratory4 &c) -> std::expected<void, boost::system::error_code> {
            ...
            res = DoSmthWithImage("lab4_sutherland_hodgman.png", [](Image &img) -> void {
                ...
                for (auto &p : subject) {
                    sub2d.push_back(p.ToPoint2D());
                }
                for (auto &p : clip) {
                    clip2d.push_back(p.ToPoint2D());
                }
                for (auto &p : result) {
                    res2d.push_back(p.ToPoint2D());
                }
            ...
            res = DoSmthWithImage("lab4_degenerate_test.png", [](Image &img) {
                std::vector<Point> degeneratePoly = {
                    {10, 10},
                    {50, 10},
                    {30, 10}
                };
            ...
            res = DoSmthWithImage(
                "lab4_complex_convex_clipping.png",
                [](Image &img) {
                    img.ResizeRgb(600, 600);
                    std::vector<Point> clipPoly = {
                        {100, 300},
            ...
            res = DoSmthWithImage(
                "lab4_star_sutherland_hodgman.png",
                [](Image &img) {
                    img.ResizeRgb(600, 600);
            ...
            res = DoSmthWithImage(
                "lab4_cb_vertex_and_collinear.png",
                [](Image &img) {
                    img.ResizeRgb(400, 400);
                    std::vector<Point> clip = {
                        {100, 100},
            ...
            res = DoSmthWithImage(
                ...
                [](Image &img) {
                    ...
                    std::vector<Point> degenerateClip = {
                        { 50, 200},
                        {350, 200},
                        {200, 200}
                    };
        ...
        [](const Homework1 &c) -> std::expected<void, boost::system::error_code> {
            ...
            auto res = DoSmthWithImage(
                c.output_filename + "_original.png",
                [&](Image &img) { DrawPolygonEdges(img, poly, {100, 100, 100}); },
                500,
                500);
