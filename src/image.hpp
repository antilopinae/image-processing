...
public:
    ...
    explicit ImageBase(size_t width = 0, size_t height = 0) : width_(width), height_(height), data_(width * height) {}

    ImageBase(size_t width, size_t height, ImVector<value_type> &&data)
        : width_(width), height_(height), data_(std::forward<ImVector<value_type> >(data))
    {
    }
...
public:
    ...
    explicit ImageRGB(size_t width = 0, size_t height = 0, Type type = Type::kGray)
        : image_(GetWidthFrom(width, type), height)
    {
    }

    ImageRGB(size_t width, size_t height, vector_gray &&data, Type type = Type::kGray)
        : image_(GetWidthFrom(width, type), height, std::move(data))
    {
    }
