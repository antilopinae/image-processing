...
public:
    ...
    ImVector(size_t count, size_t capacity, value_type *data) noexcept : size_(count), capacity_(capacity), data_(data)
    {
    }
    ...
    {
        if (index >= size_) {
            throw std::out_of_range("ImVector::[]: index out of range");
        }
        return data_[index];
    }
    ...
    {
        if (index >= size_) {
            throw std::out_of_range("ImVector::at: index out of range");
        }
        return data_[index];
    }
